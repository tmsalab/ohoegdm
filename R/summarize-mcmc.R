...
summarize_model = function(x) {
    ...
    } else if (class_x == "matrix" && type_x == "double" && n_dim_x == 2) {
        # Handles the 2D matrix case
        summarize_2d_matrix_output(x)
    } else if(class_x == "array" && type_x == "double" && n_dim_x == 3) {
        # Handles the 3D cube case
        summarize_3d_array_output(x)
    } else if(class_x == "array" && type_x == "double" && n_dim_x == 4) {
        # Handles the 4D cube case
        summarize_4d_array_output(x)
    } else {
        # Handles the 4D cube case made up of a 1D list of cubes.
        stop("Unable to create 4D cube output from a list. Please send an inquiry to the developers if needed.")
    }
}
